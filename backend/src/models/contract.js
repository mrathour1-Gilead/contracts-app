...
const flattenContractData = (data, existing = {}) => {
  ...
  Object.entries(SEARCH_CONFIG).forEach(
    ([section, fields]) => {
  ...
  result.currentExpirationDate =
    sanitizeValue(
      data?.generalTerms?.currentExpirationDate?.value
    ) ??
    existing?.currentExpirationDate ??
    null;
...
export const bulkUploadContracts = async (
  data,
  auth
) => {
  const now = new Date();
  ...
  return await db.sequelize.transaction(
    async (t) => {
      ...
      await db.Contract.bulkCreate(items, {
        transaction: t,
      });
...
export const updateContract = async (
  ...
  return await db.sequelize.transaction(
    async (t) => {
      const contract = await db.Contract.findByPk(
        id,
        {
          transaction: t,
          lock: t.LOCK.UPDATE,
      ...
      if (
        !section ||
        !(section in contract.dataValues)
      ) {
      ...
      const changes = generateAuditChanges(
        oldSection,
        newSection,
        section
      );
      ...
      const flat = flattenContractData(
        data,
        contract
      );
...
export const fetchContracts = async ({
  ...
  const { rows, count } =
    await db.Contract.findAndCountAll({
      where,
      limit,
      offset: (page - 1) * limit,
      order: [
